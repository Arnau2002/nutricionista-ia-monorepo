...
def get_products_by_category_dia(list_categories, ruta):
    ...
    for index, stringCategoria in enumerate(list_categories):
        print(f"{index+1}/{len(list_categories)} - Procesando los productos de la categoría {stringCategoria}.")
        page = 1

        while True:
            url = f"{URL_PRODUCTS_BY_CATEGORY_DIA}{stringCategoria}?page={page}&size=20"
            response = requests.get(url, headers=HEADERS_REQUEST_DIA)

            if response.status_code != 200:
                print(f"Error en la página {page} de la categoría {stringCategoria}")
                break
            ...
            df_productos = pd.json_normalize(plp_items, sep="_")
            ...
            selected_columns = ['object_id', 'display_name', 'prices_price', 'prices_price_per_unit', 'prices_measure_unit',
            ...
            df_page = df_productos[selected_columns].rename(columns=renamed_columns)
            df_products = pd.concat([df_products, df_page], ignore_index=True)

            print(f"Página {page} procesada ({len(plp_items)} productos).")
            page += 1
